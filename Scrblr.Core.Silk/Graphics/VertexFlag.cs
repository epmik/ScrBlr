...
namespace Scrblr.Core
{
    ...
    public static class VertexFlagExtensions
    {
        ...
        private static Dictionary<VertexFlag, uint>? _vertexElementCountDictionary = null;
        ...
        public static uint ElementCount(this VertexFlag vertexFlag)
        {
            if (_vertexElementCountDictionary == null)
            {
                BindVertexElementCountDictionary();
            }

            return vertexFlag.GetAttribute<ElementCountAttribute>().Count;
        }

        private static void BindVertexElementCountDictionary()
        {
            var values = Enum.GetValues<VertexFlag>();

            _vertexElementCountDictionary = new Dictionary<VertexFlag, uint>(values.Length);

            foreach (var v in values)
            {
                _vertexElementCountDictionary.Add(v, v.ElementCount());
            }
        }
