...
namespace Scrblr.LearnOpenTK
{
    ...
    public class Texture
    {
        ...
        public unsafe static Texture LoadFromFile(GL gl, string path)
        {
            ...
            using (Stream stream = File.OpenRead(path))
            {
                ...
                fixed (byte* ptr = image.Data)
                    GL.TexImage2D(GLEnum.Texture2D, 0, InternalFormat.Rgba, (uint)image.Width, (uint)image.Height, 0, GLEnum.Rgba, GLEnum.UnsignedByte, ptr);
            }
        ...
        public Texture(GL gl, uint glHandle)
        {
            GL = gl;
            Handle = glHandle;
        }
