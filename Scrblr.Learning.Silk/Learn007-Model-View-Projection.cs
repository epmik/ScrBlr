...
{
    //[Sketch(Name = "Learn007-Model-View-Projection")]
    public class Learn007ModelViewProjection : AbstractSketch
    {
        ...
        {
            return Matrix4x4.CreateTranslation(ViewPosition);
        }
        ...
        //{
        //    Gl.ClearColor(new Vector4D<float>(r, g, b, a));
        //}
        ...
        public unsafe void Load()
        {
            ...
            Gl.BufferData(BufferTargetARB.ArrayBuffer, (nuint)(2 * _positionColor1.Length * sizeof(float)), IntPtr.Zero, BufferUsageARB.StaticDraw); //Setting buffer data.

            var size = _positionColor1.Length * sizeof(float) + _positionColor2.Length * sizeof(float);
            var offset = 0;

            fixed (void* v = &_positionColor1[0])
            {
                size = _positionColor1.Length * sizeof(float);
                Gl.BufferSubData(BufferTargetARB.ArrayBuffer, (nint)offset, (nuint)size, v); //Setting buffer data.
                offset += size;
            }
            fixed (void* v = &_positionColor2[0])
            {
                size = _positionColor2.Length * sizeof(float);
                Gl.BufferSubData(BufferTargetARB.ArrayBuffer, (nint)offset, (nuint)size, v); //Setting buffer data.
                offset += size;
            }
            //Gl.BufferSubData(GLEnum.ArrayBuffer, (IntPtr)0, _positionColor1.Length * sizeof(float), _positionColor1);
            //Gl.BufferSubData(GLEnum.ArrayBuffer, (IntPtr)(_positionColor1.Length * sizeof(float)), _positionColor2.Length * sizeof(float), _positionColor2);
            ...
            Gl.VertexAttribPointer(0, 3, GLEnum.Float, false, 6 * sizeof(float), null);
...
void main()
{
    oColor = vec4(ioColor, 1.0);
            ...
            _shader = new Shader20220413(Gl, vertexShaderSource, fragmentShaderSource);
        ...
        public void Render()
        {
            ...
            model = model * Matrix4x4.CreateTranslation(0.0f, 0.0f, 0.0f);
