...
{
    [Sketch(Name = "Sketch006-render-to-multiple-frame-buffers")]
    public class Sketch006 : AbstractSketch
    {
...
";
        ...
        public void Render()
        {
            if (_secundaryGraphicsContext != null)
            {
                _secundaryGraphicsContext.Bind();

                _secundaryGraphicsContext.ClearBuffers();
            }

            Graphics.Disable(EnableFlag.DepthTest);

            Graphics.PushMatrix();
            Graphics.Rectangle().Color(1f, 0f, 0f).Size(Width / 2);
            Graphics.PopMatrix();
            ...
            if (_secundaryGraphicsContext != null)
            {
                ...
                _secundaryGraphicsContext.SaveFrame($"saves/{GetSketchName()}{DateTime.Now.ToString(".yyyyMMdd.HHmmss.ffff")}.png");

                _secundaryGraphicsContext?.Dispose();

                _secundaryGraphicsContext = null;
        ...
        public void KeyUp(KeyboardKeyEventArgs a)
        {
            ...
            else if (a.Key == Keys.S)
            {
                if (_secundaryGraphicsContext == null)
                {
                    _secundaryGraphicsContext = new GraphicsContext(
                        Width,
                        Height,
                        Graphics.ColorBits,
                        Graphics.DepthBits,
                    ...
                    _secundaryGraphicsContext.ActiveCamera(Graphics.ActiveCamera());
                    _secundaryGraphicsContext.ModelMatrix(Graphics.ModelMatrix());
                }
            }
        }
